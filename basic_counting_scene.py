...
class CountingScene(Scene):
    ...
    def introduceNumber(self, number):
        ...
        self.play(LaggedStart(*[FadeIn(block) for block in blocks], lag_ratio=0.5), run_time=2.5)
        self.wait(0.2)
        
        for i, block in enumerate(blocks, start=1):
            count = Integer(i)
            count.move_to(block.get_center() + DOWN*0.7)
            self.play(block.animate.set_color(YELLOW), run_time=0.2)
            self.play(Write(count), run_time=0.2)
            self.wait(0.2)
            self.play(block.animate.set_color(BLUE), run_time=0.2)
